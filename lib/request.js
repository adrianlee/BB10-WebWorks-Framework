    ...
    return {
        ...
        response : {
            send : function (code, data) {
                ...
                this._webview.notifyOpen(sid, code, "OK");
                this._webview.notifyHeaderReceived(sid, "Access-Control-Allow-Origin", "*");
                this._webview.notifyHeaderReceived(sid, "Access-Control-Allow-Headers", "Content-Type");
                this._webview.notifyDataReceived(sid, responseText, responseText.length);
                this._webview.notifyDone(sid);
            }
        }
    };
}
...
function networkResourceRequestedHandler(value) {
    var obj = JSON.parse(value),
        ...
        message,
        sid = obj.streamId,
        origin = this._webview.originalLocation,
    ...
    if (url.match("^" + utils.getURIPrefix())) {
        ...
        server.handle(message.request, message.response, this._webview.id);
    } else {
        ...
        if (hasAccess) {
            ...
            this._webview.executeJavaScript("alert('Access to \"" + url.source + "\" not allowed')");
        }
    }
...
NetworkResourceRequestHandler = function (webview) {this._webview = webview;};
NetworkResourceRequestHandler.prototype.networkResourceRequestedHandler = networkResourceRequestedHandler;
NetworkResourceRequestHandler.prototype.unknownProtocolHandler = unknownProtocolHandler;

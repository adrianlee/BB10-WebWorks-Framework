...
function _formMessage(url, origin, sid, body, webview) {
...
function networkResourceRequestedHandler(value, webview) {
    ...
    if (url.match("^" + utils.getURIPrefix())) {
        ...
        message = _formMessage(url, origin, sid, body, webview);
        response = SUBSTITUTE_RESPONSE;
        server.handle(message.request, message.response, webview.id);
    } else {
        ...
        if (hasAccess) {
            ...
            webview.executeJavaScript("alert('Access to \"" + url.source + "\" not allowed')");
        }
    }
...
var NetworkResourceRequestHandler = (function () {
    var _webview;
    function NetworkResourceRequestHandler(webview) {
        _webview = webview;
    }
    NetworkResourceRequestHandler.prototype.networkResourceRequestedHandler = function (value) {
        networkResourceRequestedHandler(value, _webview);
    };
    NetworkResourceRequestHandler.prototype.unknownProtocolHandler = unknownProtocolHandler;
    return NetworkResourceRequestHandler;
}());
